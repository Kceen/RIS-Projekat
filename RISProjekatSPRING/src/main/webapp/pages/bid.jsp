...
<body class="w3-content" style="max-width: 1200px">
	...
	<nav class="w3-sidebar w3-bar-block w3-white w3-collapse w3-top"
		...
		<div class="w3-padding-64 w3-large w3-text-grey"
			...
			<form action="/Aukcinjo/auctionController/search" method="get">
				...
				Starting price <br>
				more than (€) 
				<input type="text" name="startingPriceMoreThan" maxlength="7" size="16"> <br>
				less than (€) 
				<input type="text" name="startingPriceLessThan" maxlength="7" size="16"> <br> <br>
				
				Current price <br>
				more than (€) 
				<input type="text" name="currentPriceMoreThan" maxlength="7" size="16"> <br>
				less than (€) 
				<input type="text" name="currentPriceLessThan" maxlength="7" size="16"> <br> <br>
				
				Starting date <br>
				after
				<input type="date" name="startingDateAfter"> <br>
				before
				<input type="date" name="startingDateBefore"> <br> <br>
	...
	<div class="w3-main" style="margin-left: 250px">
		...
		<h1>${bid.item.name}</h1>
		<h3>Starting price: ${bid.startingPrice} € </h3>
		<h2> <span style="border: 5px solid black; border-radius: 5px;"> Current price: ${bid.currentPrice} € </span> </h2>
		<h3> Auction started: <fmt:formatDate value="${bid.startingDate}" pattern = "HH:mm - dd.MM.yyyy"/> </h3>

		<p> ${bid.item.description} </p>
		
		<c:if test="${!empty loggedInUser && loggedInUser.idServiceUser != bid.item.serviceuser.idServiceUser}">
			<form action="/Aukcinjo/auctionController/raiseTheBid?idBid=${bid.idBid}" method="post">
				<h3> BID ON THIS ITEM </h3>
				Enter your price (€) <input type="number" name="newCurrentPrice" min="${bid.currentPrice}" step="10">
				
				<input type="submit" value="BID"> <br><br>
			</form>
		</c:if>
		
		<c:if test="${loggedInUser.idServiceUser == bid.item.serviceuser.idServiceUser}">
			<p> You can't bid on your auction </p>
		</c:if>
		
		<c:if test="${empty loggedInUser }">
			<p> Login or register to bid </p> 
		</c:if>
		

		<c:forEach items="${itemImages}" var="image">
			<img src="${image.imageLink}" width="49%">
		</c:forEach>
		
		<br><br>
		
		<p> Auction started by user: <a href="/Aukcinjo/serviceUserController/showUserDetails?idUser=${bid.item.serviceuser.idServiceUser}"> ${bid.item.serviceuser.username} </a> </p>
		
		<c:if test="${loggedInUser.idServiceUser == bid.item.serviceuser.idServiceUser}">
			<a href="/Aukcinjo/auctionController/endBid?idBid=${bid.idBid}"> CLICK HERE TO END THIS AUCTION </a>
		</c:if>
