...
<body class="w3-content" style="max-width: 1200px">
	...
	<div class="w3-main" style="margin-left: 250px">
		...
		<h4> ${user.username} </h4>
		<img src="${user.imageLink}" width="30%">
		
		<p> ${user.email} </p> 
		<p> ${user.city}, ${user.country} </p>
		<p> ${user.phoneNumber} </p>

		<p> Number of positive ratings: ${positiveRatingsNumber} </p>
		<p> Number of neutral ratings: ${neutralRatingsNumber} </p>
		<p> Number of negative ratings: ${negativeRatingsNumber} </p>
		<a href="/Aukcinjo/pages/ratings.jsp"> View all ratings </a> <br> <br>	
		
		<c:if test="${!empty loggedInUser && loggedInUser.idServiceUser != user.idServiceUser}">
			<a href="/Aukcinjo/serviceUserController/viewMessages?idSender=${loggedInUser.idServiceUser}&idReceiver=${user.idServiceUser}"> Send ${user.firstName} a message </a>
		</c:if>
		
		<br><br>
		
		<c:if test="${!empty loggedInUser && !empty usersToRate && loggedInUser.idServiceUser == user.idServiceUser}">
			<p> Please rate your experience with these users </p>
			
			<c:forEach items="${usersToRate}" var="user">
				<form action="/Aukcinjo/serviceUserController/rate?idSender=${loggedInUser.idServiceUser}&idReceiver=${user.idServiceUser}" method="post">
					<p> ${user.username} </p> 
		...
		<c:if test="${!empty loggedInUser && loggedInUser.idServiceUser == user.idServiceUser}">
			<form action="/Aukcinjo/serviceUserController/statistics?idUser=${loggedInUser.idServiceUser}" method="get">
				<p> View your statistics </p>
				from <input type="date" name="startingDate"> <br>
				until <input type="date" name="endDate"> <br> <br>
