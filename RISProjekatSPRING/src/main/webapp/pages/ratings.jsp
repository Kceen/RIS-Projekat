...
<body class="w3-content" style="max-width: 1200px">
	...
	<div class="w3-main" style="margin-left: 250px">
		...
		<h3> All ratings for ${user.firstName} ${user.lastName} </h3>
		<h4> ${user.username} </h4>
	
		<p> Number of positive ratings: <b style="color: green"> ${positiveRatingsNumber} </b> </p>
		<p> Number of neutral ratings: <b> ${neutralRatingsNumber} </b> </p>
		<p> Number of negative ratings: <b style="color: red"> ${negativeRatingsNumber} </b> </p>
		 
		<h3> Positive ratings </h3>
		<c:forEach items="${positiveRatings}" var="posRating">
			<p> from user ${posRating.serviceuser1.username} </p>
		</c:forEach>
		<h3> Neutral ratings </h3>
		<c:forEach items="${neutralRatings}" var="neuRating">
			<p> from user ${neuRating.serviceuser1.username} </p>
		</c:forEach>
		<h3> Negative ratings </h3>
		<c:forEach items="${negativeRatings}" var="negRating">
			<p> from user ${negRating.serviceuser1.username} </p>
		</c:forEach>
